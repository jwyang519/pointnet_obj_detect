...
def test(model, loader, focal_loss=None):
    with torch.no_grad():
        ...
        return instance_acc, table_acc, non_table_acc, total_tables, total_non_tables, avg_loss, f1
...
def main(args):
    ...
    for idx in range(len(train_dataset)):
        ...
        if label == 0:  # non-table
            sample_weights[idx] = 1.0 / num_non_table
        else:  # table
            sample_weights[idx] = 1.0 / num_table
    ...
    # Use moderate class weights with balanced sampling
    if args.use_weights:
        non_table_weight = 2.0  # Moderate weight since we're using balanced sampling
        table_weight = 1.0
        class_weights = torch.FloatTensor([non_table_weight, table_weight])
        focal_loss = FocalLoss(gamma=4.0, weight=class_weights)  # Increased gamma for harder examples
        log_string(f'Using focal loss (gamma=4.0) with class weights: [non-table: {non_table_weight}, table: {table_weight}]')
        log_string('Using balanced batch sampling')
    else:
        class_weights = None
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        with torch.no_grad():
            instance_acc, table_acc, non_table_acc, val_tables, val_non_tables, val_loss, val_f1 = test(classifier.eval(), valDataLoader, focal_loss)
            ...
            if val_f1 > best_f1:
                ...
                if patience_counter >= patience:
                    log_string(f'Early stopping triggered after {patience} epochs without improvement')
                    break
            
            log_string('Best Instance Accuracy: %f' % best_instance_acc)
            log_string('Best Table Accuracy: %f' % best_table_acc)
            log_string('Best F1 Score: %f' % best_f1)
